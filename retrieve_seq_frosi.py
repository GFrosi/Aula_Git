import sys
import argparse
from util import db_index, search_fasta_frosi
...
def main():
    ...
    parser_splice = subparsers.add_parser('splice', help='Find the range sequence')

    parser_splice.add_argument('-f', '--file', dest='file', action='store', help='A multifasta file',
                              required=False)
    parser_splice.add_argument('-r', '--range', help='The values in the form start-end space separated. '
                                                    '10-20 50-60 10-100', action='store', required=False, nargs='+')
    parser_splice.add_argument('-g', '--gene', help='A gene or chromossome name', action='store', required=False)
    ...
    if hasattr(args, 'start') and args.start is not None:   # args.start exists and has a value
        ...
        gene_name = args.gene
        gabi = search_fasta_frosi.search(fasta, start, end, gene_name)
    ...
    if hasattr(args, 'len') and args.len:   # arg.len is True
        ...
        search_fasta_frosi.len(fasta, gene_name)
    ...
    if hasattr(args, 'range') and args.range:
        ...
        search_fasta_frosi.splice(fasta, range, gene_name)
